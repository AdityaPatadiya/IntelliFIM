"""
Network Traffic Analysis and Monitoring Module
...
class NetworkMonitor:
    ...
    def _request_privileges(self):
        ...
        if system == "Linux":
            # For Linux, we can use pkexec or gksudo
            message = (
                "Network monitoring requires root privileges.\n\n"
                "Please run one of these commands:\n"
                "1. Run backend with sudo: sudo python main.py\n"
            ...
            message = (
                ...
                "Please run the backend as Administrator or install Npcap."
            )
            print(message)
            return False
            ...
            message = (
                "Network monitoring requires root privileges.\n\n"
                "Please run backend with sudo: sudo python main.py"
            )
            print(message)
            return False
    ...
    def _display_packet(self, packet_info: Dict[str, Any]) -> None:
        """Display packet information in terminal (for debugging)"""
        ...
        # Only display every 10th packet to avoid flooding
        ...
        # Format output
        output = f"{color}{symbol} [{timestamp}] {src_ip:15} → {dst_ip:15} | {protocol:4} | {size:5} bytes{RESET}"
        
        # Add port information if available
        if 'src_port' in packet_info and 'dst_port' in packet_info:
            output += f" | {packet_info['src_port']:5} → {packet_info['dst_port']:5}"
    ...
    def _add_alert(self, alert_type: str, message: str) -> None:
        ...
        print(f"{color}{symbol} ALERT [{alert_type}] {message}{RESET}")
    ...
    def _display_final_stats(self) -> None:
        ...
        print(f"{'='*60}")
        print(f"⏱️  Total monitoring time: {elapsed:.1f} seconds")
        print(f"📦 Total packets captured: {self.stats['total_packets']:,}")
        print(f"💾 Total data transferred: {self.stats['total_bytes']/1024/1024:.2f} MB")
        
        if elapsed > 0:
            avg_bandwidth = self.stats['total_bytes'] / elapsed
...
def main():
    ...
    parser.add_argument('--packets', '-p', type=int, default=1000, 
                       help="Number of packets to capture (0 for unlimited)")
    
    args = parser.parse_args()
    
    # Create and start monitor
    monitor = NetworkMonitor(interface=args.interface, packet_limit=args.packets)
